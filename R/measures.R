...
effect_measures <- function(pe, pc, var_pe, var_pc, conf_level,
  ...
  if (has_var) {
    ...
    if (ci_type == "logit" && extra_rd == 0) {
      ci_e <- prop_ci(pe, var_pe, conf_level, "logit")
      ci_c <- prop_ci(pc, var_pc, conf_level, "logit")
    ...
    se_lnrr <- sqrt(var_pe / pe^2 + var_pc / pc^2 + extra_lnrr)
    rr_lb <- rr * exp(-z * se_lnrr)
    rr_ub <- rr * exp(z * se_lnrr)
    se_lnor <- sqrt(var_pe / (pe * (1 - pe))^2 + var_pc / (pc * (1 - pc))^2 + extra_lnor)
