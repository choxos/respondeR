...
# Smallest distance a responder probability is allowed from 0 or 1 when it feeds
# a logarithm, a logit, or an inverse-variance weight. Extreme MIDs can make
# pnorm() return exactly 0 or 1, which would otherwise make log ratios, logits
# and weights non-finite and break pooling. The point estimates (proportions and
# the risk difference) are reported unclamped; only the ratio, log and variance
# computations use the clamped value.
RESP_EPS <- 1e-10

#' Clamp a probability away from 0 and 1
#' @noRd
clamp_p <- function(p, eps = RESP_EPS) {
  pmin(pmax(p, eps), 1 - eps)
}

#' Are both arms pinned to the same probability boundary?
#'
#' TRUE when every experimental and control responder probability is within
#' `eps` of 0 (no one responds), or every one is within `eps` of 1 (everyone
#' responds). The risk difference is then well defined (and near 0) but the risk
#' ratio, odds ratio and NNT are dominated by the clamp and not informative, so
#' they are reported as NA.
#' @noRd
is_boundary <- function(pe, pc, eps = RESP_EPS) {
  (all(pe <= eps) && all(pc <= eps)) ||
    (all(pe >= 1 - eps) && all(pc >= 1 - eps))
}
...
re_pool <- function(y, v, conf_level = 0.95, tau_method = c("DL", "REML"),
  ...
  if (tau_method == "DL") {
    return(dl_pool(y, v, conf_level, ci_method))
  }
  ...
  test <- if (ci_method == "hksj") "knha" else "z"
  fit <- metafor::rma(yi = y, vi = v, method = "REML", test = test, level = 100 * conf_level)
...
#' @param ci_method Random-effects interval: `"wald"` (Normal) or `"hksj"`
...
#' @noRd
dl_pool <- function(y, v, conf_level = 0.95, ci_method = c("wald", "hksj")) {
  ci_method <- match.arg(ci_method)
  k <- length(y)
  wf <- 1 / v
  ...
  if (ci_method == "hksj") {
    ...
    # that adapts to the observed dispersion of the study estimates.
    q_hk <- sum(wr * (y - est)^2) / ((k - 1) * sum(wr))
    se <- sqrt(q_hk)
    crit <- stats::qt((1 + conf_level) / 2, df = k - 1)
  } else {
    se <- sqrt(var)
...
#' Summarize one arm across studies for the median/mean/weighted methods
#'
