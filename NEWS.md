...
* `launch_responder_analysis()` -- launches the bundled **ResponderAnalysis**
  ...
  a per-study forest plot, CLES and CSV/Excel I/O).
...
* **SMD bridge.** The `"smd"` method pools the standardised mean difference
...
* The control responder proportion is now computed with the **same** pooling as
  the experimental arm (previously every method reused the *median* method's
  control proportion).
